...
},{}],"react-partitioner":[function(require,module,exports){
'use strict';
...
var Partitioner = (function (_Component) {
  _inherits(Partitioner, _Component);

  function Partitioner(props) {
    _classCallCheck(this, Partitioner);

    _get(Object.getPrototypeOf(Partitioner.prototype), 'constructor', this).call(this, props);
    ...
    var availableItems = [];
    var selectedItems = [];
    var allItems = props.items.map(this.mapItem);
    var selectedKeys = props.selectedItems.map(props.getItemKey);
    ...
    this.state = { allItems: allItems, availableItems: availableItems, selectedItems: selectedItems };
  }

  _createClass(Partitioner, [{
    key: 'mapItem',
    ...
    value: function selectItem(item) {
      var selectedItems = [].concat(_toConsumableArray(this.state.selectedItems), [item]);
      var availableItems = this.state.availableItems.filter(function (p) {
      ...
      this.props.onChange(_extends({}, eventArgs, { type: 'select' }));
    }
  }, {
    key: 'deselectItem',
    value: function deselectItem(item) {
      var availableItems = [].concat(_toConsumableArray(this.state.availableItems), [item]);
      var selectedItems = this.state.selectedItems.filter(function (p) {
        return p.key !== item.key;
      });
      var eventArgs = this.getEventArgs(item, availableItems, selectedItems);
      ...
      this.props.onChange(_extends({}, eventArgs, { type: 'deselect' }));
    }
  }, {
    key: 'getEventArgs',
    value: function getEventArgs(item, available, selected) {
      return {
        item: item.actual,
        availableItems: available.map(function (x) {
          return x.actual;
        }),
        selectedItems: selected.map(function (x) {
          return x.actual;
        })
      };
    }
